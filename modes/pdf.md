...
15. Read `name` from `config/profile.yml` → normalize to kebab-case lowercase (e.g. "John Doe" → "john-doe") → `{candidate}`
    15b. Determine `{slug}`: if this run has (or will have) a report number, `{slug}` = `{company}-{report number}` (e.g. `reddit-307`) so two roles at the same company never collide on filename. Otherwise (one-off CV, no tracker entry) `{slug}` = `{company}`. **Never use `{company}` alone when a report number exists** — parallel/same-day evaluations of different roles at the same company (e.g. two Reddit reqs evaluated in one pipeline batch) will silently overwrite each other's HTML/PDF/fact-gate output if the filename doesn't disambiguate by report number.
16. Build the render payload (see the **JSON Input Schema** below) from the tailored content — emit compact structured JSON, **not** full HTML markup — and write it to `/tmp/cv-{candidate}-{slug}.json`
17. Run: `node build-cv-html.mjs /tmp/cv-{candidate}-{slug}.json output/cv-{candidate}-{slug}.html {template}` — where `{template}` is the path printed by **Selecting the template** below (omit the argument to use the base `cv-template.html`). The script merges the payload into that template, owning every tag, CSS class, and the HTML escaping. Write to `output/` (NOT a temp dir — the recorded HTML is what the dashboard's `D` hotkey regenerates from, so it must survive temp cleanup)
18. Run the fact gate: `node verify-cv-facts.mjs output/cv-{candidate}-{slug}.html`
...
- If the user named a template this turn (e.g. "use the _modern_ template"), run:
  ...
  "experience": [
    {
      ...
      "bullets": [
        ...
        "Another quantified-impact bullet"
      ]
    }
  ],
  "projects": [
